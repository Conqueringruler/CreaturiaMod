...
namespace Creaturia.Tiles.Plants.Trees
{
    ...
    public class AshwoodSapling : ModTile
    {
        public override void SetStaticDefaults()
        {
            ...
            TileObjectData.newTile.AnchorValidTiles = new[] { ModContent.TileType<SinfulDirt>(), TileID.Gold };
      ...
      /*  public override void RandomUpdate(int i, int j)
        {
            // This code will give a chance to grow the sapling into a tree
            if (WorldGen.genRand.NextBool(2)) // We use the Random used by WorldGen. There is a 5% chance to grow
            {
                bool isPlayerNear = WorldGen.PlayerLOS(i, j); // We check if a player is near
                bool success = WorldGen.GrowTree(i, j); // Check if the tree can actually grow
                if (success && isPlayerNear)
                {
                    WorldGen.TreeGrowFXCheck(i, j); // We show the effect of it growing
                }
            }
        } */
        public override void RandomUpdate(int i, int j)
        ...
        {
            if (i % 2 == 1) // This checks if i is basically even or odd
            {
                spriteEffects = SpriteEffects.FlipHorizontally; // This just flips the graphic
            }
        }
    }
}
